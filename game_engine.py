...
from level_schema import TILE_EMPTY, TILE_GOAL, TILE_SOLID, TILE_START, TILE_TRAP
...
class LevelDevilEngine:
    ...
    def step(self, action: int) -> StepResult:
        ...
        # Vertikale Kollision
        self.on_ground = False
        if self._collides_rect(new_x, new_y):
            if self.vy > 0:
                # Landung
                new_y = float(int(new_y + PLAYER_H)) - PLAYER_H
                self.on_ground = True
            self.vy = 0.0
            new_y = self.y if self._collides_rect(new_x, new_y) else new_y
